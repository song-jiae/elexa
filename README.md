...
├── responsive.scss         # 반응형 스타일 분리
├── _variables.scss         # SCSS 변수 (색상, 폰트, 간격 등)
...
├── js/                     # JavaScript 파일들
│   ├── index.js            # 메인 페이지 로직
│   ├── products.js         # 상품 카드 생성 및 이벤트
│   ├── products-data.js    # 상품 데이터
│   ├── cart.js             # 장바구니 기능
│   ├── cart-page.js         # 장바구니 페이지 로직
│   ├── product-detail.js   # 상품 상세 페이지
│   ├── search.js           # 검색 기능
│   ├── mypage.js           # 마이페이지 탭 전환
│   ├── best-products.js    # 베스트 상품 렌더링
│   ├── recent-products.js # 최근 본 상품
│   ├── header.js           # 헤더 기능 (모바일 메뉴)
│   ├── utils.js            # 유틸리티 함수
...
1. **하나씩 배우기**: 모든 걸 한 번에 배우려고 하지 말고, 하나씩 확실히 배우기
2. **직접 만들어보기**: 강의만 보지 말고, 직접 코드를 쳐보기
3. **에러를 두려워하지 않기**: 에러가 나는 게 정상, 에러 메시지를 읽어보면 힌트가 있음
4. **구글 검색하기**: 모르는 게 있으면 검색해보기 (Stack Overflow가 친구)
5. **주석 많이 쓰기**: 나중에 내가 봐도 이해하기 쉽게
6. **작은 프로젝트부터**: 처음부터 큰 걸 만들려고 하지 말고, 작은 것부터
